...
- **Collections render through `EntityList`; lists never render blank.** Don't hand-roll the empty-vs-populated ternary per feature. The shared `EntityList<T>` (`shared/components`) takes `items`, an `emptyMessage`, an `onRefresh`, a `getKey`, and a `renderItem` — it renders a MUI `List` of `renderItem(item)` when there are entries and the shared `EmptyState` (message + Refresh button) when empty, so a successful-but-empty collection always indicates nothing was found rather than showing a bare container. `EntityList`/`EmptyState` only **notify** that refresh was clicked (`onRefresh`); the consuming component **dictates how** (e.g. passing the RTK Query `refetch`). A feature component pairs `QueryBoundary` (loading/error) with `EntityList` (empty/populated): the render-prop child returns `<EntityList items={data.items} … />`.
