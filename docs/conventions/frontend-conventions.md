...
- **Lists never render blank.** A successfully-loaded but empty list must show an indication that nothing was found — never a bare, empty container. Use the shared `EmptyState` (`shared/components`): it takes a `message` and an `onRefresh` callback and renders the message plus a Refresh button. The shared component only **notifies** that refresh was clicked (`onRefresh`); the consuming component **dictates how** the refresh happens (e.g. passing the RTK Query `refetch`). Feature components branch on the empty case inside their `QueryBoundary` render prop (`data.items.length === 0 ? <EmptyState … /> : <List … />`).
