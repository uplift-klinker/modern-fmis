...
> Two things this factory must get right (both surfaced as failing integration tests during development): (1) replacing the DbContext requires removing **all** EF Core/Npgsql service descriptors, not just `DbContextOptions<FmisDbContext>` — otherwise both providers stay registered and EF throws "Only a single database provider can be registered"; (2) the InMemory database name is computed **once** in a field, not inside the options lambda — otherwise EF rebuilds it per request scope (a fresh name each time) and data written by one request is invisible to the next.
