...
describe('client schemas', () => {
  ...
  it('accepts a create request with only a phone number', () => {
    const result = CreateClientRequestSchema.safeParse({ name: 'Acme', email: null, phoneNumber: '555-0100' });

    expect(result.success).toBe(true);
  });

  it('rejects a create request with neither email nor phone', () => {
    const result = CreateClientRequestSchema.safeParse({ name: 'Acme', email: null, phoneNumber: null });

    expect(result.success).toBe(false);
  });

  it('rejects a blank name', () => {
    const result = CreateClientRequestSchema.safeParse({ name: '', email: 'ops@acme.example', phoneNumber: null });
